...
columns_to_keep_in_table_control = ['g_IDs'] + [col for col in df.columns if col.endswith('NM')]
columns_to_keep_in_table_experimental = ['g_IDs'] + [col for col in df.columns if col.endswith('DS')]
...
ds_columns_control = [col for col in df.columns if col.endswith('NM')]
df_ds = df[['g_IDs'] + ds_columns_control]
gene_dict_control = df_ds.set_index('g_IDs')[ds_columns_control].apply(list, axis=1).to_dict()


ds_columns_experimental = [col for col in df.columns if col.endswith('DS')]
df_ds = df[['g_IDs'] + ds_columns_experimental]
gene_dict_experimental = df_ds.set_index('g_IDs')[ds_columns_experimental].apply(list, axis=1).to_dict()
