...
# for this prepare one table with columns for the list and one with where gene_id is determined as key
    ...
    gene_dict_control = df_ds_control.set_index(gene_identifier)[df_control].apply(list, axis=1).to_dict()
   
    gene_dict_experimental = df_ds_experimental.set_index(gene_identifier)[df_experimental].apply(list, axis=1).to_dict()
